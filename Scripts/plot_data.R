...
ggplot(plot.df, aes(x = Date, y = Cumulative, color = County)) + geom_line(size = 1.2) + scale_color_manual(values = cols) + xlab("2020") + ylab("COVID19 cases (cumulative)") + theme(legend.position = "right", legend.key = element_rect(fill = "#f0f0f0"), legend.background = element_rect(fill = "#ffffffaa", colour = "black"), panel.background = element_rect(fill = "white", colour = "black"), axis.text.y = element_text(colour = "black", size = 14), axis.text.x = element_text(colour = "black", size = 10), axis.title = element_text(colour = "black", size = 16), panel.grid.minor = element_line(colour = "#00000000",linetype = 3), panel.grid.major = element_line(colour = "#00000000", linetype = 3)) + scale_y_continuous(trans = log_trans(), limits = c(1, 2000)) + labs(color = "MA County")
